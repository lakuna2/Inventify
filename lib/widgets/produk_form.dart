...
class _ProductFormSheetState extends State<_ProductFormSheet> {
  ...
  static const _cats = ['Makanan', 'Minuman', 'Snack', 'Lainnya'];

  // Preview laba realtime
  double get _previewLaba =>
      (double.tryParse(_jualCtrl.text) ?? 0) - (double.tryParse(_beliCtrl.text) ?? 0);
  ...
  void dispose() {
    ...
    _beliCtrl.dispose(); _jualCtrl.dispose();
    super.dispose();
  }
    ...
    if (existing != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        ...
        action: SnackBarAction(
          label: 'Edit', textColor: Colors.white,
          onPressed: () => ProductForm.show(context, product: existing),
        ),
      ));
      return;
    }
    setState(() { _barcode = code; _scanned = true; });
  }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(_isEdit ? 'Edit Barang' : 'Tambah Barang',
              ...
              if (_scanned) ...[
                _BarcodeBadge(
                  barcode: _barcode,
                  onReset: _isEdit ? null : () => setState(() { _scanned = false; _barcode = ''; }),
                ),
                const SizedBox(height: 12),
                _field(_nameCtrl, 'Nama Barang', Icons.inventory_2_outlined),
                const SizedBox(height: 10),
                _field(_descCtrl, 'Deskripsi', Icons.notes_outlined, lines: 2),
                const SizedBox(height: 10),

                // Stok + Kategori
                Row(children: [
                  Expanded(child: _field(_stockCtrl, 'Stok', Icons.numbers_outlined, numeric: true)),
                ...
                Row(children: [
                  Expanded(child: _field(_beliCtrl, 'Harga Beli', Icons.arrow_downward_rounded, numeric: true)),
                  const SizedBox(width: 10),
                  Expanded(child: _field(_jualCtrl, 'Harga Jual', Icons.arrow_upward_rounded, numeric: true)),
                ]),
                const SizedBox(height: 10),
                ...
                _ActionButtons(
                  isEdit: _isEdit, loading: _loading,
  ...
  Widget _field(TextEditingController c, String label, IconData icon, {int lines = 1, bool numeric = false}) =>
      TextField(
        controller: c,
        maxLines: lines,
    ...
    return Container(
      ...
      child: Row(children: [
        Icon(isPositive ? Icons.trending_up_rounded : Icons.trending_down_rounded,
          color: color, size: 18),
        const SizedBox(width: 10),
        Text('Estimasi laba/unit  ', style: TextStyle(fontSize: 12, color: color)),
        const Spacer(),
        Text(_rp(laba),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Barcode', style: TextStyle(fontSize: 10, color: AppColors.textGrey)),
          Text(barcode, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textDark)),
        ])),
        if (onReset != null)
          GestureDetector(onTap: onReset,
            child: const Icon(Icons.refresh_rounded, color: AppColors.secondary, size: 18)),
      ]),
    );
  }
}
...
class _ManualBarcodeInput extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      ...
      ElevatedButton(
        onPressed: () { if (_ctrl.text.trim().isNotEmpty) onSubmit(_ctrl.text.trim()); },
...
class _ActionButtons extends StatelessWidget {
  ...
  const _ActionButtons({required this.isEdit, required this.loading, required this.onCancel, required this.onSubmit});
