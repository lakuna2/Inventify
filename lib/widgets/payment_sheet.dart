...
class _PaymentSheetContentState extends State<_PaymentSheetContent> {
  ...
  List<int> get _quickPay {
    ...
    // Uang pas: bulatkan ke atas ke ribuan terdekat
    final uangPas = ((_total / 1000).ceil() * 1000).toInt();

    final hasil = <int>{uangPas}; // pakai Set biar tidak duplikat

    for (final d in denominasi) {
      if (d >= _total) {
        hasil.add(d);
  ...
  Future<void> _bayarSekarang() async {
    ...
    try {
      ...
      ReceiptSheet.show(
        context,
        txId: id,
        items: List.from(widget.cart.items), // copy sebelum clear
        total: _total,
        bayar: _bayar,
      );
      widget.cart.clear();
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: _cukup
                        ...
                        : AppColors.habis.withValues(alpha: 0.3),
                  ),
                ),
                child: Row(
                  children: [
                    ...
                    const Spacer(),
                    Text(
                      rupiahFormat(_cukup ? _kembalian : _total - _bayar),
                      style: TextStyle(
                        fontSize: 15,
                        fontWeight: FontWeight.w700,
                        color: _cukup ? AppColors.tersedia : AppColors.habis,
...
// ═══════════════════════════════════════════════════════════════════════════
// ReceiptSheet — langsung cetak tanpa dialog pilih printer
// ═══════════════════════════════════════════════════════════════════════════
...
class ReceiptSheet {
  static void show(
    ...
    showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent,
      builder: (_) =>
          _ReceiptContent(txId: txId, items: items, total: total, bayar: bayar),
    );
  }
}
...
class _ReceiptContentState extends State<_ReceiptContent> {
  ...
  double get _kembalian => widget.bayar - widget.total;

  /// Langsung cetak — jika printer belum connect, arahkan ke Profil.
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${item.qty} x ${rupiahFormat(item.hargaJual)}',
                          style: TextStyle(
                            fontSize: 11,
                            color: AppColors.textGrey.withValues(alpha: 0.8),
                          ),
          ...
          _receiptRow('Total', rupiahFormat(widget.total), bold: true),
          const SizedBox(height: 4),
          _receiptRow('Bayar', rupiahFormat(widget.bayar)),
          const SizedBox(height: 4),
          _receiptRow(
            'Kembalian',
            rupiahFormat(_kembalian),
            valueColor: AppColors.tersedia,
            bold: true,
          ),
          ...
          // Status printer — informatif, tanpa tombol pilih
          _PrinterStatusBar(printer: _printer),
          const SizedBox(height: 10),
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(_printing ? 'Mencetak...' : 'Cetak Resi'),
...
/// Hanya menampilkan status koneksi printer — tidak ada tombol pilih.
class _PrinterStatusBar extends StatelessWidget {
  final PrinterService printer;
  const _PrinterStatusBar({required this.printer});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: connected
            ? AppColors.tersedia.withValues(alpha: 0.07)
            : AppColors.stokTipis.withValues(alpha: 0.07),
        borderRadius: BorderRadius.circular(10),
        border: Border.all(
          color: connected
              ? AppColors.tersedia.withValues(alpha: 0.25)
              : AppColors.stokTipis.withValues(alpha: 0.25),
        ),
      ),
      child: Row(
        children: [
          Icon(
            ...
            size: 15,
            color: connected ? AppColors.tersedia : AppColors.stokTipis,
          ),
          const SizedBox(width: 7),
          Expanded(
            child: Text(
              connected
                  ? 'Printer: ${printer.connectedDeviceName}'
                  : 'Printer belum terhubung — atur di Profil',
              style: TextStyle(
                fontSize: 12,
                color: connected ? AppColors.tersedia : AppColors.stokTipis,
                fontWeight: FontWeight.w500,
